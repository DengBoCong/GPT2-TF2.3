...
def block(units, deep, num, dropout):
    ...
    attention_state = attention(deep, num)(inputs=[inputs, inputs, inputs, mask])
    attention_state = tf.keras.layers.LayerNormalization(epsilon=1e-6)(attention_state + inputs)
    outputs = tf.keras.layers.Dense(units=units, activation="relu")(attention_state)
    outputs = tf.keras.layers.Dense(units=deep)(outputs)
    outputs = tf.keras.layers.Dropout(rate=dropout)(outputs)
    outputs = tf.keras.layers.LayerNormalization(epsilon=1e-6)(outputs + attention_state)
...
def decoder(num_layers, num_heads, units, deep, dropout):
    ...
    for i in range(num_layers):
        outputs = block(units, deep, num_heads, dropout)(inputs=[outputs, mask])
    return tf.keras.Model(inputs=[inputs, mask], outputs=outputs)
...
def gpt2(vocab_size, num_layers, units, deep, num_heads, dropout):
    inputs = tf.keras.Input(shape=(None,))
    outputs = encoder(vocab_size=vocab_size, deep=deep, dropout=dropout)(inputs)

    mask = tf.keras.layers.Lambda(data_utils.creat_look_ahead_mask, output_shape=(1, None, None))(inputs)
